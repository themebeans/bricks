	...
	foreach ( $meta_box['fields'] as $field ) {
		...
		switch ( $field['type'] ) {
				...
				jQuery(function($) {
					...
					$('#<?php echo esc_js( $field['id'] ); ?>_button').on('click', function(e) {
						...
						frame.toolbar.get('view').set({
							insert: {
								style: 'primary',
								text: '<?php esc_html_e( 'Insert', 'bricks' ); ?>',
							...
							insert: {
								...
								click: function() {
									var models = frame.state().get('selection'),
										...
										this.el.innerHTML = '<?php esc_html_e( 'Saving...', 'bricks' ); ?>';
			...
			?>
				...
				jQuery(function($) {
					...
					$('#bean_images_upload').on('click', function(e) {
						...
						//FIRST FRAME
						var options = {
							title: '<?php esc_html_e( 'Create Gallery', 'bricks' ); ?>',
							state: 'gallery-edit',
							frame: 'post',
							selection: selection
						};
						//IF FRAME IS THERE
						if( frame || selection ) {
							options['title'] = '<?php esc_html_e( 'Edit Gallery', 'bricks' ); ?>';
						}

						frame = wp.media(options).open();

						// CUSTOMIZE VIEWS
						frame.menu.get('view').get('cancel').el.innerHTML = '<?php esc_html_e( 'Cancel', 'bricks' ); ?>';
						frame.menu.get('view').get('gallery-edit').el.innerHTML = '<?php esc_html_e( 'Edit Gallery', 'bricks' ); ?>';
						...
						function overrideGalleryInsert() {
							frame.toolbar.get('view').set({
								insert: {
									style: 'primary',
									text: '<?php esc_html_e( 'Save Gallery', 'bricks' ); ?>',
			...
			<?php
				...
				$thumbnail_output = '';
				$button_text      = ( $meta ) ? __( 'Edit Gallery', 'bricks' ) : $field['std'];
			if ( $meta ) {
				$field['std']     = __( 'Edit Gallery', 'bricks' );
				$thumbs           = explode( ',', $meta );
				$thumbnail_output = '';
			...
			case 'checkbox':
                ...
                <input type="checkbox" id="' . $field['id'] . '" name="bean_meta[' . $field['id'] . ']" value="on"' . $val . ' /><span>' . __( 'Yes, please do', 'bricks' ) . '</span>';
				echo '</td>';
				break;
		}
